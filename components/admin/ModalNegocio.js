// components/admin/ModalNegocio.js
// ─────────────────────────────────────────────────────
// Modal para CREAR o EDITAR un negocio
//
// Props:
//   negocio  → null (crear) | objeto (editar)
//   onClose   → cerrar modal
//   onGuardado → callback con el negocio guardado
// ─────────────────────────────────────────────────────
...
export default function ModalNegocio({ negocio = null, onClose, onGuardado }) {
  const { user } = useAuth();
  ...
  const [form, setForm] = useState({
    ...
    tipo: negocio?.tipo || "Tienda",
    imagen_url: negocio?.imagen_url || "",
    direccion: negocio?.direccion || "",
    telefono: negocio?.telefono || "",
    whatsapp: negocio?.whatsapp || "",
    horario: negocio?.horario || "",
    activo: negocio?.activo !== undefined ? negocio.activo : true,
  });
